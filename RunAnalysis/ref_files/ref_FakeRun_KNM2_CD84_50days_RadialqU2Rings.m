function TritiumObject = ref_FakeRun_KNM2_CD84_50days_RadialqU2Rings(varargin)
TDMode = 'DataTBD';
...
[~,RingPixList] = Ring2PixelHalfCombi(1:2,1:148);
nPixRing = cell2mat(cellfun(@(x) numel(x),RingPixList,'UniformOutput',0));
qUOffset = [0,0.1];
qU = mean(D.qU,2)+qUOffset;
qUfrac = repmat(mean(D.qUfrac,2),1,numel(qUOffset));
% ---------------------------------------------------------------------- %
% WGTS
p = inputParser;
...
p.addParameter('MACE_Ba_T',6.3*1e-04.*ones(1,numel(qUOffset)),@(x)isfloat(x) && x>0);
...
% General
p.addParameter('TimeSec',50*24*60*60.*ones(1,numel(qUOffset)),@(x)isfloat(x));
...
p.addParameter('BKG_RateRingSec',0.210/117.*nPixRing');
p.addParameter('FPD_Segmentation','RING',@(x) ismember(x,{'OFF','SINGLEPIXEL','MULTIPIXEL','RING'}));
p.addParameter('FPD_RingMerge','Half',@(x) ismember(x,{'Default','Full','Half','None'}));
p.addParameter('PixList',1:148);
p.addParameter('RingList',1:numel(qUOffset));
...
FPD_RingMerge            = p.Results.FPD_RingMerge;
