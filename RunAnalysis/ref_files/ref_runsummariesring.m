function TritiumObject = ref_runsummariesring(RunNr,ring,varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% CONFIGURATION FILE
% First Tritium Runs
%
...
%Load RunSummary
D = importdata([num2str(RunNr),'mpix.mat']); 
...
p.addParameter('TD',['Run',num2str(RunNr),'mpix']);

% Binning
p.addParameter('nTeBinningFactor',25,@(x)isfloat(x) && x>0);
...
p.addParameter('BKG_RateAllFPDSec',0,@(x)isfloat(x) && x>=0);
