function TritiumObject = ref_stackedrunsring(StackFileName,ex2,ring,varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% CONFIGURATION FILE
% First Tritium Runs
%
...
%Load RunSummary
D = importdata([StackFileName,'mpix',ex2,'.mat']); 
...
p.addParameter('TD',[StackFileName,'mpix',ex2]);

% Binning
p.addParameter('nTeBinningFactor',20,@(x)isfloat(x) && x>0);
...
p.addParameter('Mode','DataTBD',@(x)ismember(x,{'DataTBD', 'Sim', 'Read'}));
...
p.addParameter('FPD_Segmentation','RING',@(x) ismember(x,{'RING'}));
...
opt_wgts = {...
    'WGTS_MolFrac_TT',WGTS_MolFrac_TT,... %WGTS_MolFrac_TT
    'WGTS_MolFrac_DT',WGTS_MolFrac_DT,...
    'WGTS_MolFrac_HT',WGTS_MolFrac_HT,... %WGTS_MolFrac_TT
...
opt_fpd = {...                                                  
    ...
    'FPD_Ring',ring,'FPD_MeanEff',0.95};
