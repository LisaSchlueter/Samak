function TritiumObject = ref_RunAnalysis_FT(DataType,D,RunNr_or_StackFileName,TwinName, PixList,RingList,RingMerge,varargin)%,varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% CONFIGURATION FILE -  First Tritium
...
switch DataType
    ...
    case 'Real'
        ...
        if isfloat(RunNr_or_StackFileName)
            RunSummaryFileName = [RunPrefix,num2str(RunNr_or_StackFileName),'.mat'];
        elseif ischar(RunNr_or_StackFileName)
            RunSummaryFileName = [RunPrefix,RunNr_or_StackFileName,'.mat'];
        end  
    % BASED ON FAKE MC DATA RUNS    
    case 'Fake'
        %        RunPrefix = 'Fake1Run';
        ...
        if isfloat(RunNr_or_StackFileName)
            RunSummaryFileName = [RunPrefix,num2str(RunNr_or_StackFileName),mpix,'.mat'];
        elseif ischar(RunNr_or_StackFileName)
            RunSummaryFileName = [RunPrefix,RunNr_or_StackFileName,mpix,'.mat'];
        end
end
% FAKE
...
% MTD
% p.addParameter('TD',strrep(RunSummaryFileName,'.mat',''));
% Warning: Modif Thierry
...
% p.addParameter('TD','StackCD100allex2');
% Warning: End Modif Thierry
...
switch DataType
    ...
    case 'Real'
        ...
        p.addParameter('MACE_Bmax_T',4.2,@(x)isfloat(x) && x>0);
        p.addParameter('MACE_Ba_T',D.MACE_Ba_T,@(x)all(isfloat(x)) && all(x>0));
        p.addParameter('ISCS','Aseev',@(x)ismember(x,{'Aseev','Theory'}));
        % TD Reading Mode
        ...
        % ---------------------------------------------------------------------- %
        % FT
        p.addParameter('DTFSD','DOSS',@(x)ismember(x,{'OFF','DOSS','BlindingKNM1'}));
        p.addParameter('HTFSD','SAENZ',@(x)ismember(x,{'OFF','SAENZ','BlindingKNM1'}));
        p.addParameter('TTFSD','SAENZ',@(x)ismember(x,{'OFF','DOSS','SAENZ','BlindingKNM1'}));
        % ---------------------------------------------------------------------- %
        p.addParameter('ELossFlag','Abdurashitov',@(x)ismember(x,{'Aseev','Abdurashitov','CW_GLT','KatrinD2'}));
        ...
        p.addParameter('BKG_RateAllFPDSec',1e-03);
    ...
    case 'Fake'
        ...
        p.addParameter('FPD_PixList',Fake1RunSimObj.SimFakeObj_i.PixelList,@(x)isfloat(x) && all(x)>0);
        % Transmission Function
        ...
        % Multipixel
        p.addParameter('FPD_Pixel',Fake1RunSimObj.SimFakeObj_i.FPD_Pixel,@(x)isfloat(x) && all(x>=0));
        % Ring
        p.addParameter('FPD_Ring',Fake1RunSimObj.SimFakeObj_i.FPD_Ring,@(x)isfloat(x) && all(x>=0));
end
...
opt_wgts = {...
    ...
    'NIS',11}; %Thierry WARNING 13/2/2019 

opt_mace = {...
    'MACE_Bmax_T',MACE_Bmax_T,...
    'MACE_Ba_T',MACE_Ba_T,...
    'MACE_Ba_Setting','6.0G'};
...
if isempty(BKG_RateAllFPDSec)
    BKG_RateAllFPDSec = numel(FPD_PixList)/148*GetBackground('MACE_Ba_T',MACE_Ba_T,...
        'WGTS_B_T',WGTS_B_T,...
        'FPD_ROIlow',FPD_ROIlow);
 end
opt_bkg = {...
