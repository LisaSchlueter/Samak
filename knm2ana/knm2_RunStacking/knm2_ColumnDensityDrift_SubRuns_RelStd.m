% calculate Twim simulation with some relative column density drift
% fit with average response function
% uniform FPD

RelDrift  = -1e-02.*[0.5]; % peak-to-peak
...
savename = sprintf('%sknm2_ColumnDensityDrift_%.3gSubRunDrift_Bpng-%.1fmucpsPers_%s_%.0feV_%s_%s_%s_%s.mat',...
    savedir,RelDrift,BKG_PtSlope*1e6,DataType,range,strrep(freePar,' ',''),chi2,AnaFlag,FSDFlag);
...
if exist(savename,'file')
    ...
    WGTS_CD_MolPerCm2_SubRun = WGTS_CD_MolPerCm2_i+linspace(-WGTS_CD_MolPerCm2_i.*RelDrift/2,WGTS_CD_MolPerCm2_i.*RelDrift/2,nqUFit);
