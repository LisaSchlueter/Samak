function TritiumObject = ref_FakeRun_KNM2_RFcomparison(varargin)
TDMode = 'RFcomparison';

% use MTD from a KNM2 run
filename = [getenv('SamakPath'),'tritium-data/mat/Knm2/56341.mat']; %take MTD from 1 (randomly chosen) KNM1 run
...
% WGTS
p = inputParser;
p.addParameter('WGTS_CD_MolPerCm2',4.2*1e17,@(x)isfloat(x) && x>0);
...
% Theory
p.addParameter('ISCS','Theory',@(x)ismember(x,{'Aseev','Theory','Edep'}));
...
p.addParameter('qUfrac',1,@(x)isfloat(x));

% MACE
p.addParameter('MACE_Bmax_T',4.238,@(x)isfloat(x) && x>0);
p.addParameter('MACE_Ba_T',6.308*1e-04,@(x)isfloat(x) && x>0);
