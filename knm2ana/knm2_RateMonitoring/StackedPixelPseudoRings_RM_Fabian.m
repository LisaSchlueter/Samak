...
if strcmp(Mode,'Ringwise')
    ...
    for j=1:3
        ...
        CratePeriodWise{j} = Crate2{j,1}+Crate2{j,2}+Crate2{j,3}+Crate2{j,4};
    ...
    Crate = [CratePeriodWise{1} CratePeriodWise{2} CratePeriodWise{3}];
    writematrix(Crate,'Potential_in_mV.txt');
...
if strcmp(Mode,'Periodwise')
    ...
    %
        ...
        % Slow Control Data: HV setting
        qUCorrRW2       = (R.SingleRunData.qU_RM);
        ...
        % Correct for KNM1 Radial Effect if any
        count  = count ./ mean(KNM1correction);
    ...
    for j=1:3
        ...
        qUmeanRW        = mean(R.SingleRunData.qU_RM);
        ...
        % Ref = Period2 - Average qU correction  
        HVcorrCPSperPixel = (qUmeanRW - qUmeanRW2) * 6.3032 * numel(R.PixList);

        % Correct for activity and qU 1
        cf     = R.RMRateErosCorrectionqUActivity;
        Crate1{j} = (rateRaw.*cf./mean(Activity{j}).*ActivityRW2 + HVcorrCPSperPixel - RefPeriodRW2CPS);
        % Correct for activity and qU
        R.RMCorrection('saveplot','OFF','QAplots','OFF');
        count  = R.SingleRunData.TBDIS_RM + HVdriftPerPixel.*numel(R.PixList).*R.SingleRunData.qUfrac_RM.*R.SingleRunData.TimeSec;
        ...
        % j = RW Period
        % i = PSR
        Crate2{j}                =  (rate./mean(Activity{j}).*ActivityRW2 + HVcorrCPSperPixel - RefPeriodRW2CPS);

        CrateEquivalentmV{j}    =  -(rate./mean(Activity{j}).*ActivityRW2 + HVcorrCPSperPixel - RefPeriodRW2CPS(1)./737.8 * 1e3);
