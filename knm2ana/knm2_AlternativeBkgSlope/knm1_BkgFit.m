...
PrettyFigureFormat;
legend([hdata hfit],...
    sprintf('KNM1 Data: %.0f counts in %.0f sec',sum(A.RunData.TBDIS(A.exclDataStart:end)),sum(A.RunData.qUfrac(A.exclDataStart:end).*A.RunData.TimeSec)),...
...
grid on
publish_figurePDF(1,'BKG_KNM1.pdf');
