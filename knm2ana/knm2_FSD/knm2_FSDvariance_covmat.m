% dummy script to get relative uncertainty on variance uncertainties
nSamples = 5e4;
...
% result
fprintf('Total var = %.1f eV^2 +-%.1f eV^2 (%.3g%%) \n',mean(VarTotal_samples),std(VarTotal_samples),1e2*std(VarTotal_samples)/var(E,Prob))
fprintf('GS var = %.3f eV^2 +-%.2g eV^2  (%.2g%%) \n',mean(VarGS_samples),std(VarGS_samples),1e2*std(VarGS_samples)/var(E(E<=8),Prob(E<=8)))
fprintf('40 eV var = %.1f eV^2 +-%.1f eV^2 (%.2f%%) \n',mean(Var40_samples),std(Var40_samples),1e2*std(Var40_samples)/var(E(E<=40),Prob(E<=40)))
