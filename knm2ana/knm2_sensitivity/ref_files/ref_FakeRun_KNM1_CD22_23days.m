function TritiumObject = ref_FakeRun_KNM1_CD22_23days(varargin)
TDMode = 'DataTBD';

% use MTD from a KNM2 run
filename = [getenv('SamakPath'),'tritium-data/mat/Knm1/51936.mat']; %take MTD from 1 (randomly chosen) KNM1 run
D = importdata(filename);

% ---------------------------------------------------------------------- %
% WGTS
p = inputParser;
p.addParameter('WGTS_CD_MolPerCm2',0.22*5*1e17,@(x)isfloat(x) && x>0);
...
p.addParameter('DTFSD','Sibille0p5eV',@(x)ismember(x,{'OFF','DOSS','BlindingKNM1','SibilleFull','Sibille0p5eV'}));
p.addParameter('HTFSD','Sibille0p5eV',@(x)ismember(x,{'OFF','SAENZ','BlindingKNM1','SibilleFull','Sibille0p5eV'}));
...
% General
p.addParameter('TimeSec',23*24*60*60,@(x)isfloat(x));
...
p.addParameter('BKG_RateAllFPDSec',0.290);
