...
R.ComputeCM('SysEffects',struct('TASR','ON','TCoff_OTHER','OFF','FSD','ON','RF_EL','ON','RF_BF','ON','RF_RX','ON','Stack','ON'),...
...
nqU_used = size(R.ModelObj.TBDIS(R.exclDataStart:end,:),1);             % number of subruns, which are NOT excluded
exclIndex = sort(reshape(repmat(R.exclDataStart:R.ModelObj.nqU,[nrings,1])+[0:nrings-1]'.*R.ModelObj.nqU,[nqU_used*nrings,1]));
close all
