function TritiumObject = ref_FakeRun_KNM2_CD84_50days_RadialqU(varargin)
TDMode = 'DataTBD';
...
[~,RingPixList] = Ring2PixelCombi(1:4,1:148);
 nPixRing = cell2mat(cellfun(@(x) numel(x),RingPixList,'UniformOutput',0));
qUOffset = [0,0.05,0.1,0.15];
qU = mean(D.qU,2)+qUOffset;
qUfrac = repmat(mean(D.qUfrac,2),1,4);
% ---------------------------------------------------------------------- %
% WGTS
p = inputParser;
...
p.addParameter('MACE_Ba_T',6.3*1e-04.*ones(1,4),@(x)isfloat(x) && x>0);
...
% General
p.addParameter('TimeSec',50*24*60*60.*ones(1,4),@(x)isfloat(x));
...
p.addParameter('FPD_RingMerge','Full',@(x) ismember(x,{'Default','Full'}));
p.addParameter('PixList',1:148);
p.addParameter('RingList',1:4);
