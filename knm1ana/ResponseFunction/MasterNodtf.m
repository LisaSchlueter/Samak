function [MasterNodtfTe MasterNodtfRF] = MasterNodtf()
...
MasterNodtfTe       = zeros(3000,148);
MasterNodtfRF       = zeros(numel(qu),3000,148);
for i=1:numel(qu)
    masterRFfile = [rfdir 'MasterRF_NoDTF_' num2str(qu(i)) '.000000.dat'];
    rftmp = importdata(masterRFfile);
    disp(masterRFfile);
    % read Kinetic Energy from first file
    if i == 1
        for p=1:148
        MasterNodtfTe(:,p) = rftmp(:,1) + ElectricPotentialCorrection(p);
        end
    end
    % read pixelwise response functions
    MasterNodtfRF(i,:,:) =  rftmp(:,2:149);
end
save('MasterNodtfRF.mat','MasterNodtfTe','MasterNodtfRF');
end
