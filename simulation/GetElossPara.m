function [parKatrinT2, errKatrinT2] = GetElossPara(varargin)
%
% Read ELoss Parameters from TOF Data Fit
% Last Updated: 06/04/2020
%

p = inputParser;
p.addParameter('ElossFit','M19',@(x)ismember(x,{'M19','A20'}));
p.parse(varargin{:});
ElossFit = p.Results.ElossFit;

switch ElossFit
    case 'M19'
        d = importdata([getenv('SamakPath'),'inputs/ELossFunction/ELoss_KatrinT2_parametrization_May19.dat']);
    case 'A20'
        d = importdata([getenv('SamakPath'),'inputs/ELossFunction/ELoss_KatrinT2_parametrization_April20.dat']);
end
