...
classdef WGTSMACE < FPD & handle %!dont change superclass without modifying parsing rules!
    ...
    methods
        ...
        function out    = ComputeISProb(obj,varargin)
            ...
            GetSamakPath;
            p = inputParser;
            p.addParameter('WGTS_DensityProfile','Table',@(x)ismember(x,{'Table','File'}));
        ...
        function MaceTF    = ComputeMaceTF(obj,te,qu,varargin)
           ...
           % if isscalar(varargin{1}); varargin = {'pixel',varargin{1}}; end
            ...
            p.addParameter('gammaFacApprox',1.018,@(x)isfloat(x) && x>=0);   % Approximation for Relativist Transmission (Gamma+1)/2=1.018
...
%             f = @(tdeg) interp1(tmpA,tmpS,tdeg*pi/180);
        ...
        function [fscatn,fscatnE] = ComputeELossFunction(obj,varargin)
            ...
            if ~isempty(file_logic) && all(E==tmp.E) && strcmp(obj.recomputeRF,'OFF') && strcmp(LoadOrSaveEloss,'ON')% if one exists and has correct binning
                fscatnE = tmp.fscatnE;
                fscatn  = tmp.fscatn;
                if strcmp(Debug,'ON')
