...
if nargin < 3, 
    error('mystats:f_cdf:TooFewInputs','Requires three input arguments.'); 
end

[errorcode x v1 v2] = distribution_check(3,x,v1,v2);

if errorcode > 0
    error('mystats:f_cdf:InputSizeMismatch',...
...
if any(~isfinite(v1(:)) | ~isfinite(v2(:)))
   ...
   if any(k)
      p(k) = chi2_cdf(v1(k).*x(k),v1(k));
   end
   k = find(x > 0 & ~t & ~isfinite(v1) & v1>0 & isfinite(v2));
   if any(k)
      p(k) = 1 - chi2_cdf(v2(k)./x(k),v2(k));
   end
