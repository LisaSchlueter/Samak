function x = student_inv_cdf(p,v);
%STUDENT_INV_CDF   Inverse of Student's T cumulative distribution function (cdf).
%   X=STUDENT_INV_CDF(P,V) returns the inverse of Student's T cdf with V degrees 
...
if nargin < 2, 
    error('mystats:student_inv_cdf:TooFewInputs','Requires two input arguments.'); 
end

[errorcode p v] = distribution_check(2,p,v);

if errorcode > 0
    error('mystats:student_inv_cdf:InputSizeMismatch',...
...
if any(k)
    z = beta_inv_cdf(2*(1-p(k)),v(k)/2,0.5);
    x(k) = sqrt(v(k) ./ z - v(k));
end

k = find(p < 0.5 & p > 0 & ~isnan(x) & v < 1000);
if any(k)
    z = beta_inv_cdf(2*(p(k)),v(k)/2,0.5);
    x(k) = -sqrt(v(k) ./ z - v(k));
end
