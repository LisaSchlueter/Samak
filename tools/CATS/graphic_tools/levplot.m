function levplot(s)
% Leverage plot of the fit performed by CATS
%
% levplot(s) plots the leverages of the fit performed by CATS where s is
...
clf;
barh(s.leverage(1:nobs),'facecolor',rgb('CadetBlue'));
if nsys==0
    colormap('default')
else
    colormap(lines(nsys));
...
hold on;
for i=1:nsys
    barh(nobs+i,s.leverage(nobs+i),'FaceColor',cmap(i,:));
end
hold off;
box on;
axis([ 0 max(s.leverage)*1.2 0 length(s.leverage)+1]);
grid off;
...
line(2*(nsys+npar)/(nobs+nsys)*[1 1],[1-1 length(s.leverage)+1],...
    'LineWidth',2,'LineStyle','--','Color',.5*ones(3,1));
line((nsys+npar)/(nobs+nsys)*[1 1],[1-1 length(s.leverage)+1],...
