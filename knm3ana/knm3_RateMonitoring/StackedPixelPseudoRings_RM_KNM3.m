...
if strcmp(Mode,'Ringwise')
    %
    % Reference Rate: KNM3a
    %
        DataType  = 'Real';
        RunAnaArg = {'RunList','KNM3a','DataType',DataType,...
        ...
        for i=1:A.nRings
            ...
            qUCorr_a       = (R.SingleRunData.qU_RM(1,:));
            qUmean_a(i)    = mean(qUCorr_a);
        ...
        Activity_a  =  mean(R.SingleRunData.WGTS_MolFrac_TT'+0.5*R.SingleRunData.WGTS_MolFrac_HT'+0.5*R.SingleRunData.WGTS_MolFrac_DT')...
            .*mean(R.SingleRunData.WGTS_CD_MolPerCm2);

        %
        % End of Reference Rate: KNM3a
        %
    ...
    for j=1:2
        ...
        for i=1:A.nRings
            ...
            HVcorrCPSperPixel{j,i} = (qUmeanRW{j,i} - qUmean_a(i)) * 6.3032 * numel(R.PixList);

            % Correct for activity and qU 1
            cf(i,:)     = R.RMRateErosCorrectionqUActivity;
            Crate1{j,i} = (rateRaw{j,i}.*cf(i,:)./mean(Activity{j}).*Activity_a + HVcorrCPSperPixel{j,i});% - (RefPeriodRW2CPS(1)./numel(A.MultiObj(1).PixList).*numel(R.PixList));
            ...
            % j = RW Period
            % i = PSR
            Crate2{j,i} =  (rate{j,i}./mean(Activity{j}).*Activity_a + HVcorrCPSperPixel{j,i});% - (RefPeriodRW2CPS(1)./numel(A.MultiObj(1).PixList).*numel(R.PixList));
...
if strcmp(Mode,'Periodwise')
    ...
    %
        ...
        % Slow Control Data: HV setting
        qUCorr_a       = (Ref.SingleRunData.qU_RM);
        qUmean_a       = mean(qUCorr_a);
        ...
        Activity_a  =  mean(Ref.SingleRunData.WGTS_MolFrac_TT'+0.5*Ref.SingleRunData.WGTS_MolFrac_HT'+0.5*Ref.SingleRunData.WGTS_MolFrac_DT')...
    ...
    for j=1:2
        ...
        % Ref = Period2 - Average qU correction  
        HVcorrCPSperPixel{j} = (qUmeanRW{j} - qUmean_a) * 6.3032 * numel(R.PixList);

        % Correct for activity and qU 1
        cf     = R.RMRateErosCorrectionqUActivity;
        Crate1{j} = (rateRaw.*cf./mean(Activity{j}).*Activity_a + HVcorrCPSperPixel{j});
        % Correct for activity and qU
        ...
        % Ref = Period2 PSR1
        % j = RW Period
        Crate2{j}                =  (rate./mean(Activity{j}).*Activity_a + HVcorrCPSperPixel{j});

        if strcmp(Corr,'Fabian')
            CrateEquivalentmV{j}    =  -(rate./mean(Activity{j}).*Activity_a + HVcorrCPSperPixel{j})./737.8 * 1e3;
        elseif strcmp(Corr,'Thierry')
            CrateEquivalentmV{j}    =  -(rateRaw.*cf./mean(Activity{j}).*Activity_a + HVcorrCPSperPixel{j})./737.8 * 1e3;
        end
