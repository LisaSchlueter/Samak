function TritiumObject = ref_KNM2_KATRIN_IsoStatMTD_Bkg0mcps_new(varargin)
% MTD with same statistic below E0! for background region same stat as before

TDMode = 'DataTBD';

% use configuration from KNM2 - except MTD
IsoStatMTD   = importdata([getenv('SamakPath'),'ksn2ana/ksn2_MTD/ref_files/KSN2IsoStatMTD_new.mat']);
qU = IsoStatMTD.qU;
qUfrac = IsoStatMTD.qUfrac;
TimeSec = IsoStatMTD.TimeSec;
DataKNM2 = IsoStatMTD.RunData;
% ---------------------------------------------------------------------- %
% WGTS
p = inputParser;
...
p.addParameter('BKG_RateAllFPDSec',1e-06);
