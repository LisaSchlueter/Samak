...
% x=linspace(-2*R1,2*R1,1000);
% y=linspace(-2*R1,r2+2*R2,1000);
% for i=1:numel(x)
%     for j=1:numel(y)
%         U(i,j)=-G.*m1./(sqrt(x(i).^2+y(j).^2))-G.*m2./(sqrt(x(i).^2+(y(j)-r2).^2));
%         if x(i)^2+y(j)^2<R1^2 || (y(j)-r2)^2+x(i)^2<R2^2
%             U(i,j)=0;
%         end
%     end
% end
