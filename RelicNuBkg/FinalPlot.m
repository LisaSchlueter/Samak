...
mnuSq=[0 sqrt(0.2) sqrt(0.4) sqrt(0.6) sqrt(0.8) 1];
eta=[3.96e11 4.24e11 4.36e11 4.48e11 4.6e11 4.7e11];
load('./RelicNuBkg/SensVsMnuSq_KNM2_Prompt_[0_4].mat');
eta2=Sensitivities;
mnuSq2=sqrt(ScanPoints);
mnuSq2=mnuSq;
eta2=[1.4e11 1.6e11 1.75e11 1.9e11 1.95e11 1.96e11];
...
plot(mnuSq2,eta2,'LineWidth',3,'Color','red','LineStyle','--');
...
text(0.2,6e11,'KRN1 Twins','FontSize',14,'FontWeight','bold');
text(1.6,4e10,'KRN2 Twins','FontSize',14,'FontWeight','bold');
text(0.4,2e10,'KATRIN Sensitivity limit (Bkg rate 130 mcps)','FontSize',14,'FontWeight','bold');
...
a=semilogy(A.Te-A.Q,A.TBDDS+1e-50,'LineWidth',2);
hold on;
b=semilogy(A.Te-A.Q,A.TBDDS_R,'LineWidth',2);
c=semilogy(B.Te-B.Q,B.TBDDS-B.TBDDS_R+1e-50,'LineWidth',2);
...
ZeroPoint = min(min(Chi2));
hold on;
[q,t]=contour(mnuScanPoints,etaScanPoints,(Chi2-ZeroPoint).',[0 4.61]);
t.LineWidth = 2;
[r,u]=contour(mnuScanPoints,etaScanPoints,(Chi2-ZeroPoint).',[0 5.99]);
u.LineWidth = 2;
[s,v]=contour(mnuScanPoints,etaScanPoints,(Chi2-ZeroPoint).',[0 9.21]);
v.LineWidth = 2;
v.Color=[0 0.8 0.8];
[~,w]=contour(mnuScanPoints(15:end),etaScanPoints,(Chi2(15:end,:)-ZeroPoint).',[0 9.21]);
w.LineWidth=4;
w.Color=[0 0.5 0.5];
ylabel('\eta','FontSize',12);
xlabel('m_{\nu}^{2} (eV^{2})','FontSize',12);
...
fig14a=figure('Renderer','painters');
set(fig14a, 'Units', 'normalized', 'Position', [0.001, 0.001,0.45, 0.6]);
load('./RelicNuBkg/Chi2Scans/RelicChi2Scan_Twin_BiasmnuSq0_SystON_range40_KNM2_Prompt_2D_Real.mat');
ZeroPoint = Chi2(mnuScanPoints==min(abs(mnuScanPoints)),etaScanPoints==min(abs(etaScanPoints)));
hold on;
[q,t]=contour(mnuScanPoints,etaScanPoints,(Chi2-ZeroPoint).',[0 4.61]);
t.LineWidth = 2;
[r,u]=contour(mnuScanPoints,etaScanPoints,(Chi2-ZeroPoint).',[0 5.99]);
...
xlabel('m_{\nu}^{2} (eV^{2})','FontSize',12);
zlabel('\Delta\chi^2','FontSize',12);
text(0.25,7e10,'90% C.L.','FontSize',14,'Rotation',12);
text(0.25,8e10,'95% C.L.','FontSize',14,'Rotation',12);
text(0.25,10e10,'99% C.L.','FontSize',14,'Rotation',12);
xlim([0,1]);
ylim([1.7e10,1.5e11]);
s(1,1)=2;s(1,105)=2;s(1,107)=2;s(1,62)=2;s(1,2)=2;%r(1,17)=-2;q(1,17)=-2;s(2,32)=-2e11;s(1,1)=3;s(2,1)=-8e11;
j=fill(s(1,:),s(2,:),[0 0.5 0.5],'FaceAlpha',0.2);
i=fill(r(1,2:end),r(2,2:end),[0 1 1],'FaceAlpha',0.2);
h=fill(q(1,2:end),q(2,2:end),[50 148 216]/255,'FaceAlpha',0.2);
PrettyFigureFormat;
...
PullChi2  = Chi2Combi;
for i=1:numel(mnuScanPoints)
    PullChi2(i,:)=Chi2Combi(i,:)+exp(-mnuScanPoints(i)/1e-3);
end
ZeroPoint = min(min(Chi2));
fig16=figure('Renderer','painters');
set(fig16, 'Units', 'normalized', 'Position', [0.001, 0.001,0.45, 0.6]);
hold on;
[q,t]=contour(mnuScanPoints,etaScanPoints,(Chi2Combi-ZeroPoint).',[0 4.61]);
t.LineWidth = 2;
[r,u]=contour(mnuScanPoints,etaScanPoints,(Chi2Combi-ZeroPoint).',[0 5.99]);
u.LineWidth = 2;
[s,v]=contour(mnuScanPoints,etaScanPoints,(Chi2Combi-ZeroPoint).',[0 9.21]);
v.LineWidth = 2;
v.Color=[0 0.8 0.8];
[~,w]=contour(mnuScanPoints(1:end),etaScanPoints,(Chi2Combi(1:end,:)-ZeroPoint).',[0 9.21]);
w.LineWidth=4;
w.Color=[0 0.5 0.5];
ylabel('\eta','FontSize',12);
xlabel('m_{\nu}^{2} (eV^{2})','FontSize',12);
zlabel('\Delta\chi^2','FontSize',12);
text(0.25,0.7e11,'90% C.L.','FontSize',14,'Rotation',10);
text(0.25,0.9e11,'95% C.L.','FontSize',14,'Rotation',10);
text(0.25,1.1e11,'99% C.L.','FontSize',14,'Rotation',10);
xlim([-0,1]);
ylim([-1e11,1.5e11]);
s(1,1)=-2;%r(1,17)=-2;q(1,17)=-2;s(2,32)=-2e11;s(1,1)=3;s(2,1)=-8e11;
j=fill(s(1,:),s(2,:),[0 0.5 0.5],'FaceAlpha',0.2);
i=fill(r(1,2:end),r(2,2:end),[0 1 1],'FaceAlpha',0.2);
h=fill(q(1,2:end),q(2,2:end),[50 148 216]/255,'FaceAlpha',0.2);
%a=annotation('rectangle',[0.13 0.13 0.368 0.795],'FaceColor','magenta','FaceAlpha',.2);
b=annotation('rectangle',[0.13 0.13 0.775 0.32],'FaceColor','magenta','FaceAlpha',.2);
text(-0.8,4.5e11,{'Unphysical','neutrino mass'},'FontSize',20,'Color','red','Rotation',-30);
text(0.0173,-2.4e10,'X','FontWeight','bold','FontSize',16,'Rotation',90);
d=annotation('textarrow',[0.3 0.15],[0.5 0.379],'String','Global best fit');
%text(0.05,-5.8e10,'X','FontWeight','bold','FontSize',16,'Rotation',90);
%d=annotation('textarrow',[0.6 0.52],[0.4 0.29],'String','Global best fit');
d.FontSize=16;
PrettyFigureFormat;
...
fig16a=figure('Renderer','painters');
set(fig16a, 'Units', 'normalized', 'Position', [0.001, 0.001,0.45, 0.6]);
hold on;
[q,t]=contour(mnuScanPoints,etaScanPoints,(Chi2Combi-ZeroPoint).',[0 4.61]);
t.LineWidth = 2;
[r,u]=contour(mnuScanPoints,etaScanPoints,(Chi2Combi-ZeroPoint).',[0 5.99]);
...
xlabel('m_{\nu}^{2} (eV^{2})','FontSize',12);
zlabel('\Delta\chi^2','FontSize',12);
text(0.25,0.8e11,'90% C.L.','FontSize',14,'Rotation',10);
text(0.25,1e11,'95% C.L.','FontSize',14,'Rotation',10);
text(0.25,1.2e11,'99% C.L.','FontSize',14,'Rotation',10);
xlim([0,1]);
ylim([3e9,1.5e11]);
s(1,s(1,:)>4)=-2;r(1,r(1,:)>4)=-2;q(1,q(1,:)>4)=-2;
j=fill(s(1,:),s(2,:),[0 0.5 0.5],'FaceAlpha',0.2);
i=fill(r(1,2:end),r(2,2:end),[0 1 1],'FaceAlpha',0.2);
h=fill(q(1,2:end),q(2,2:end),[50 148 216]/255,'FaceAlpha',0.2);
PrettyFigureFormat;
...
fig17=figure('Renderer','painters');
set(fig17, 'Units', 'normalized', 'Position', [0.001, 0.001,0.45, 0.6]);
load('./RelicNuBkg/Chi2Scans/RelicChi2Scan_Twin_BiasmnuSq0_SystON_range40_KNM1_2D_Real.mat');
hold on;
[q,t]=contour(mnuScanPoints,etaScanPoints,(Chi2-GlobalChi2Min).',[0 4.61]);
t.LineWidth = 2;
...
v.LineWidth = 2;
v.Color=[0 0.8 0.8];
[~,w]=contour(mnuScanPoints(5:end),etaScanPoints,(Chi2(5:end,:)-GlobalChi2Min).',[0 9.21]);
w.LineWidth=4;
w.Color=[0 0.5 0.5];
ylabel('\eta','FontSize',12);
xlabel('m_{\nu}^{2} (eV^{2})','FontSize',12);
zlabel('\Delta\chi^2','FontSize',12);
text(0.4,5.5e11,'90% C.L.','FontSize',14,'Rotation',10);
text(0.4,6.1e11,'95% C.L.','FontSize',14,'Rotation',10);
text(0.4,6.7e11,'99% C.L.','FontSize',14,'Rotation',10);
xlim([0,1]);
ylim([0,8e11]);
s(1,63)=-2;r(1,r(1,:)>4)=-2;q(1,q(1,:)>4)=-2;
j=fill(s(1,:),s(2,:),[0 0.5 0.5],'FaceAlpha',0.2);
i=fill(r(1,2:end),r(2,2:end),[0 1 1],'FaceAlpha',0.2);
h=fill(q(1,2:end),q(2,2:end),[50 148 216]/255,'FaceAlpha',0.2);
text(0.785,3.7e11,'X','FontWeight','bold','FontSize',16,'Rotation',90);
text(0.65,4.1e11,'Global best fit','FontSize',16);
PrettyFigureFormat;
hold off;

fig18=figure('Renderer','painters');
set(fig18, 'Units', 'normalized', 'Position', [0.001, 0.001, 0.5, 0.37]);
Y=[2e14 1e13 7e11 1.12e11 1.29e11 1e6];
Yerr=[0 0 1.4e11 3.7e10 3.3e10 0];
FitStyleArg = {'o','Color','k','LineWidth',1.0,'MarkerFaceColor',rgb('Red'),'MarkerSize',8,'MarkerEdgeColor',rgb('Red')};
errorbar([1 2 3 4 5 6.5],Y,Yerr,FitStyleArg{:});
xticks([1 2 3 4 5 6.5]);
xticklabels({'Los Alamos','Troitsk','KRN1','KRN2','KRN1+2','Theory limit'});
ylabel('\eta','FontSize',12);
ax=gca;
ax.YScale='log';
xlim([0.5 7]);
...
Lob99 = [Lob99 Lob99];
Rob91 = [Rob91 Rob91];
CombiResult = [1.12e11 1.26e11 1.37e11 1.44e11 1.44e11 1.34e11];
%area(linspace(-sqrt(2),2,2),Lob99,1e16,'FaceColor',[0 0.75 0.75]);
fig19=figure('Renderer','painters');
set(fig19, 'Units', 'normalized', 'Position', [0.001, 0.001, 0.6, 0.6]);
hold on;
area(linspace(-sqrt(2),2,2),HM05,1e16,'FaceColor',[0 0.75 0.75]);
area(linspace(-sqrt(2),2,2),Rob91,'FaceColor',[0 0.5 0.5]);
%plot(mnuSq,eta,'LineWidth',3,'Color','red');
plot([0 sqrt(0.2) sqrt(0.4) sqrt(0.6) sqrt(0.8) 1],CombiResult,'LineWidth',3,'Color','red');
%plot(mnuSq2,eta2,'LineWidth',3,'Color','red','LineStyle','--');
plot(sqrt(ScanPoints),Sensitivities.*1.8,'LineWidth',3,'Color','red','LineStyle',':');
%plot(linspace(-sqrt(2),2,2),KFM10,'LineWidth',2,'Color',[0.9290 0.6940 0.1250]);
plot(Faes13(:,1),Faes13(:,2),'LineWidth',2,'LineStyle','--');
plot(LVV08lower(:,1),LVV08lower(:,2));
plot(LVV08upper(:,1),LVV08upper(:,2));
...
grid on;
ax.Layer='bottom';
text(0.4,1e15,'Robertson et al.','FontSize',14);
text(0.35,5e13,'Hwang et al., Lobashev et al.','FontSize',14);
%text(1.6,1e12,'Lobashev et al.','FontSize',14);
text(0.35,3e11,'KRN1/2 combined result','FontSize',14,'FontWeight','bold');
%text(1.6,4e10,'KRN2 Twins','FontSize',14,'FontWeight','bold');
text(0.2,8e9,'KATRIN Sensitivity limit (Bkg rate 130 mcps)','FontSize',14,'FontWeight','bold');
%text(0.1,1e9,'Kaboth et al.: KATRIN sensitivity limit (design Bkg)','FontSize',14);
text(0.65,3e6,'Fässler et al.','FontSize',14);
text(0.5,2e3,'Lazauskas et al.','FontSize',14);
text(0.5,10,sprintf('Ringwald & Wong \n (extrapolated)'),'FontSize',14);
xlabel('m_{\nu} (eV)');
ylabel('\eta (99% C.L.)');
xlim([0 1]);
PrettyFigureFormat;
...
if strcmp(saveplot,'ON')
    ...
    SaveName16 = 'CombiFit.pdf';
    SaveName17 = 'KRN1_2D.pdf';
    SaveName18 = 'SummaryPlot.pdf';
    SaveName16a= 'CombiFit_wrttNullHyp.pdf';
    SaveName14a= 'KRN2_wrttNullHyp.pdf';
    SaveName19 = 'OverviewPlot.png';
    ...
    export_fig(fig16, [SaveDir,SaveName16]);
    export_fig(fig17, [SaveDir,SaveName17]);
    export_fig(fig18, [SaveDir,SaveName18]);
    export_fig(fig19, [SaveDir,SaveName19],'-m10');
    export_fig(fig1,  [SaveDir,SaveName1],'-m2');
    export_fig(fig1a, [SaveDir,SaveNamea],'-m2');
    export_fig(fig14a,[SaveDir,SaveName14a]);
    export_fig(fig16a,[SaveDir,SaveName16a]);
end