function myobj = ref_run40260(varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% CONFIGURATION FILE
...
p.addParameter('TimeSec',1860,@(x)isfloat(x) && x>0);
p.addParameter('WGTS_CD_MolPerCm2',5e17*0.82);
p.addParameter('WGTS_B_T',2.52);
p.addParameter('MACE_Bmax_T',4.2);
p.addParameter('MACE_Ba_T',15e-4);

p.addParameter('Mode','Read',@(x)ismember(x,{'DataTBD', 'Sim', 'Read'}));
p.addParameter('TD','VFT2down');
p.addParameter('BKG_Flag','ON',@(x)ismember(x,{'ON','OFF','XmasData'}));
p.addParameter('BKG_Type','FLAT',@(x)ismember(x,{'FLAT','SLOPE'}));
p.addParameter('BKG_RateAllFPDSec',3.364635822262940779e-01*0.8243243243,@(x)isfloat(x) && x>=0);
...
opt_fpd = {...
    'FPD_Segmentation','OFF','FPD_Pixel',1,'FPD_MeanEff',0.9*0.8243243243};
