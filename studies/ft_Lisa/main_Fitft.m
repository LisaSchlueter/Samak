...
%% Settings
saveplot = 'ON';
chi2 = 'CM'; %CM, CMFrac, Stat
VFTRuns = [40257];%:40259,40263:40266 40263];%, 40263,40264];%];
fix = 0;
% Column Density Scan?
rhodScan = 'OFF';
% qU-Scan?
exclData = 'OFF';
exclnPoints = 12; %12exclude up to (exclnPoints-1) Points
exclDataStop = 9999; % 9999== take all point until qUmin
if strcmp(exclData,'OFF')
    exclnPoints = 1;   
end
%Start 200eV below the endpoint? 
start200eV = 'OFF'; %only when no qU-Scan

%Init
par  = zeros(4,exclnPoints);
err  = zeros(4,exclnPoints);
chi2min = zeros(1,exclnPoints);
dof = zeros(1,exclnPoints);
...
FitCM = Data(:,2).*cm.CovMatFracCombi.*Data(:,2)' + CM_Stat;
FitCMFrac = cm.CovMatFracCombi + CMFrac_Stat;
...
par(:,i) = F.RESULTS{1};
err(:,i) = F.RESULTS{2};
chi2min(i) = F.RESULTS{3};
dof(i) = numel(M.qU(exclDataStart:end))-4+fix;
switch rhodScan
    case 'ON'
      parRD(:,rd) =  par(:,i);
      errRD(:,rd) =  err(:,i);
...
myfitleg = sprintf(' Fit: \\chi2 / dof=%.2f/%.0f \n m^2=%.3f \t\\pm %.3f eV \n E0=%.3f \t\\pm %.3f eV \n B=%.3f \t\\pm %.3f mcps\n N=%.3f \t\\pm %.3f',...
    chi2min(i),dof(i),((M.mnuSq_i+par(1,i))),err(1,i),(par(2,i)),err(2,i),...
    (M.BKG_RateSec_i+par(3,i))*1e3,err(3,i)*1e3,par(4,i),err(4,i));

leg1 = legend([hdata hfit1],'Data',myfitleg,'Location','NorthEast') ;
leg1.FontSize = 12;
grid on
...
if strcmp(saveplot,'ON')
    ...
    save_name = sprintf('./plots/Run%u/FitRun%u_chi2-%s-%s.eps',RunNr,RunNr,chi2,excl);
        case 'ON'
    save_name = sprintf('./plots/RhoD_Scan/Spectra/FitRun%u_chi2-%s-%s-%g-molPercm2.eps',RunNr,chi2,excl,WGTS_CD_MolPerCm2_local(rd));     
    end
...
switch rhodScan
    case 'ON'
        save_as = sprintf('Run%u_RhoDScan_%s.mat',RunNr,chi2);
        save(['./plots/RhoD_Scan/',save_as]);
        ...
        set(fig12, 'Units', 'normalized', 'Position', [0.9, 0.9, 1.5 ,1.2]);
        subplot(2,4,1);
        ...
        errorbar(WGTS_CD_MolPerCm2_local./D.WGTS_CD_MolPerCm2*100, 1+parRD(4,:),errRD(4,:),...
            's-', 'Color',orange,'LineWidth',1.5);
        xlabel('Column Density (%)');
        ylabel('N');
        xlim([rhomin rhomax]);
        ...
        grid on;
        PrettyFigureFormat;
        legend(sprintf('100%% Column Density =\n   %.3g mol/cm2',D.WGTS_CD_MolPerCm2));
        ...
        plot(WGTS_CD_MolPerCm2_local./D.WGTS_CD_MolPerCm2*100, 1-chi2cdf(chi2RD,dof),...
            's-', 'Color',orange,'LineWidth',2);
        xlabel('Column Density (%)');
        ylabel('p-value');
        xlim([rhomin rhomax]);
        ...
        save_name = sprintf('./plots/RhoD_Scan/RhoDScan%s-Run%u.eps',chi2,RunNr);
...
switch exclData
    case 'ON'
        save_as = sprintf('./plots/qU_Scan/qUScan_%s_Run%u.mat',chi2, RunNr);
        save(save_as);
        qUmin = min(M.Q_i-Data(1:exclnPoints,1));
        qUmax = max(M.Q_i-Data(1:exclnPoints,1));
      
        fig12 = figure(12345); %Endpoint
        set(fig12, 'Units', 'normalized', 'Position', [0.1, 0.1, 1.7 ,0.88]);
        subplot(2,4,1);
        errorbar(M.Q_i-Data(1:exclnPoints,1), par(2,:),err(2,:),...
           's-', 'Color',orange,'LineWidth',1.5);
        xlabel('18575 - qU_{min} (eV)');
        ylabel('E_0 - 18575 (eV)');
        xlim([qUmin qUmax]);     
        grid on;
        PrettyFigureFormat;             
        ...
        subplot(2,4,2); % Neutrino mass
        errorbar(M.Q_i-Data(1:exclnPoints,1), (M.mnuSq_i+par(1,:)),err(1,:),...
             's-', 'Color',orange,'LineWidth',1.5);
        xlabel('18575 - qU_{min} (eV)');
        ylabel('m^2 (eV)')
        xlim([qUmin qUmax]);
        ...
        subplot(2,4,5); % Background
        errorbar(M.Q_i-Data(1:exclnPoints,1), (M.BKG_RateSec_i+par(3,:))*1e3,err(3,:)*1e3,...
            's-', 'Color',orange,'LineWidth',1.5);
        xlabel('18575 - qU_{min} (eV)');
        ylabel(sprintf('BKG (m cps)'));
        xlim([qUmin qUmax]);
        ...
        subplot(2,4,6); % Normalization
        errorbar(M.Q_i-Data(1:exclnPoints,1), 1+par(4,:),err(4,:),...
            's-', 'Color',orange,'LineWidth',1.5);
        xlabel('18575 - qU_{min} (eV)');
        ylabel('N');
        xlim([qUmin qUmax]);
        ...
        plot(M.Q_i-Data(1:exclnPoints,1), chi2min./dof,...
            's-', 'Color',orange,'LineWidth',2);
        xlabel('Fit Range: 18575 - qU_{min} (eV)');
        ylabel('\chi^2/dof');
        xlim([qUmin qUmax]);
        grid on;
        PrettyFigureFormat;
        title(sprintf('Very First Tritium Run %u\nSamak Fit Range Scan (%s) Results',RunNr,chi2));
        set(gca,'YAxisLocation','right');  
        set(gca,'FontSize',12);
       
         subplot(2,4,[7,8]); % p-value
        plot(M.Q_i-Data(1:exclnPoints,1), 1-chi2cdf(chi2min,dof)',...
            's-', 'Color',orange,'LineWidth',2);
        xlabel('Fit Range: 18575 - qU_{min} (eV)');
        ylabel('p-value');
        xlim([qUmin qUmax]);
        ...
        set(gca,'FontSize',12);
        if strcmp(saveplot,'ON')       
        save_name = sprintf('./plots/qU_Scan/qUScanRun%u_chi2-%s.eps',RunNr,chi2);
        publish_figure(fig12, save_name);        
        end  
