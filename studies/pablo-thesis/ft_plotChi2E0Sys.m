...
subplot(1,3,1)
chi2_s = SPAll.TableShortSys(:,9); dof_s = SPAll.TableShortSys(1,10);
hold on
...
subplot(1,3,2)
chi2_s = SPAll.TableMedSys(:,9); dof_s = SPAll.TableMedSys(1,10);
hold on
[~,Nbins,Wbins] = nhist(chi2_s);
binWidth = Wbins(2) - Wbins(1);
x_N = linspace(min(chi2_s),max(chi2_s),...
...
subplot(1,3,3)
chi2_s = SPAll.TableLongSys(:,9); dof_s = SPAll.TableLongSys(1,10);
hold on
[~,Nbins,Wbins] = nhist(chi2_s);
binWidth = Wbins(2) - Wbins(1);
x_N = linspace(min(chi2_s),max(chi2_s),...
...
suptitle('\chi^2 Dist. Sys. All Runs');


export_fig('plots/Chi2E0Sys.pdf','-pdf')
