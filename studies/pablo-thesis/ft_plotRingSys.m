...
E0_ring_s = R.TableShortSys(:,2) + E0_i - E_sys_R_s;
E0_ring_m = R.TableMedSys(:,2) + E0_i - E_sys_R_m;

 
FPDViewer([E0_ring_s;nan;nan])
title('First Tritium - Ring-wise Short Sys. E0 - <E_0> (eV)')
publish_figurePDF(gcf,'plots/FPDRingShortSys.pdf')
%export_fig('plots/FPDRingShortSys.png','-png')

FPDViewer([E0_ring_m;nan;nan])
title('First Tritium - Ring-wise Med. Sys. E0 - <E_0> (eV)')
publish_figurePDF(gcf,'plots/FPDRingMedSys.pdf')
%export_fig('plots/FPDRingMedSys.png','-png')
...
% PLOT 1
subplot(2,2,1)
E0Ring_s = R.TableShortSys(:,2) + E0_i - E_sys_R_s;
E0Ring_s_e = R.TableShortSys(:,8);
hold on
...
% PLOT 2
subplot(2,2,2)
E0Ring_s = R.TableMedSys(:,2) + E0_i - E_sys_R_m;
E0Ring_s_e = R.TableMedSys(:,8);
hold on
errorb(xRing,E0Ring_s,E0Ring_s_e);
h1 = plot(xRing,E0Ring_s,'sk','MarkerFaceColor','black');
plot(xRing,zeros(1,length(xRing)),'Color','red','LineStyle','-','LineWidth',3);
...
chi2_s = R.TableShortSys(:,13); dof_s = R.TableShortSys(1,14);
hold on
...
% PLOT 4
subplot(2,2,4)
chi2_s = R.TableMedSys(:,13); dof_s = R.TableMedSys(1,14);
hold on
[~,Nbins,Wbins] = nhist(chi2_s);
binWidth = Wbins(2) - Wbins(1);
x_N = linspace(min(chi2_s),max(chi2_s),...
...
export_fig('plots/RingSys.pdf','-pdf');
