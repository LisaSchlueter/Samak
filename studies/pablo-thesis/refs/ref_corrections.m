function TritiumObject = ref_corrections(varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
p.addParameter('WGTS_MolFrac_TT',0.9,@(x)isfloat(x) && x>0);
% p.addParameter('WGTS_MolFrac_TT_SubRun',D.WGTS_MolFrac_TT_SubRun);
p.addParameter('WGTS_MolFrac_DT',0.05,@(x)isfloat(x) && x>0);
% p.addParameter('WGTS_MolFrac_DT_SubRun',D.WGTS_MolFrac_DT_SubRun);
p.addParameter('WGTS_MolFrac_HT',0.05,@(x)isfloat(x)&& x>0);
...
p.addParameter('TTFSD','OFF',@(x)ismember(x,{'OFF','DOSS','SAENZ'}));
...
p.addParameter('qUmax',18580,@(x)isfloat(x) && all(x>=0));
p.addParameter('qUmin',18545,@(x)isfloat(x) && all(x>=0));
...
FiniteExtChargeFlag=p.Results.FiniteExtChargeFlag;
EEexchangeFlag  = p.Results.EEexchangeFlag;
ScreeningFlag   = p.Results.ScreeningFlag;
WintFiniteSizeFlag= p.Results.WintFiniteSizeFlag;
RecoilCoulombFlag= p.Results.RecoilCoulombFlag;
...
opt_corr = {...
    ...
    'FiniteExtChargeFlag',FiniteExtChargeFlag,...
    'EEexchangeFlag',EEexchangeFlag,...    
    'ScreeningFlag',ScreeningFlag,...
    'WintFiniteSizeFlag',WintFiniteSizeFlag,...
    'RecoilCoulombFlag',RecoilCoulombFlag};
