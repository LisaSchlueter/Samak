...
%     BuildTable(1);
    ...
    %SPix = load('SinglePixelResultsThesis');
%end
...
E0_pixel_s = SPix.TableShortSys(:,2) + E0_i - E_sys_SPix_s;
E0_pixel_m = SPix.TableMedSys(:,2) + E0_i - E_sys_SPix_m;

FillerPixels(PixelList)=E0_pixel_s;
E0_pixel_s = FillerPixels;
FPDViewer([E0_pixel_s])
title('First Tritium - pixel-wise Short Sys. E0 - <E_0> (eV)')
publish_figurePDF(gcf,'plots/FPDpixelShortSys.pdf')
%export_fig('plots/FPDpixelShortSys.pdf','-pdf')

FillerPixels(PixelList)=E0_pixel_m;
E0_pixel_m = FillerPixels;
FPDViewer([E0_pixel_m])
title('First Tritium - pixel-wise Med. Sys. E0 - <E_0> (eV)')
publish_figurePDF(gcf,'plots/FPDpixelMedSys.pdf')
%export_fig('plots/FPDpixelMedSys.pdf','-pdf')
...
% PLOT 1
subplot(2,2,1)
E0pixel_s = SPix.TableShortSys(:,2) + E0_i - E_sys_SPix_s;
E0pixel_s_e = SPix.TableShortSys(:,8);
hold on
...
% PLOT 2
subplot(2,2,2)
E0pixel_s = SPix.TableMedSys(:,2) + E0_i - E_sys_SPix_m;
E0pixel_s_e = SPix.TableMedSys(:,8);
hold on
errorb(xpixel,E0pixel_s,E0pixel_s_e);
h1 = plot(xpixel,E0pixel_s,'sk','MarkerFaceColor','black');
plot(xpixel,zeros(1,length(xpixel)),'Color','red','LineStyle','-','LineWidth',3);
...
chi2_s = SPix.TableShortSys(:,13); dof_s = SPix.TableShortSys(1,14);
hold on
...
hold off
xlabel('\chi2 short (E_0 - 200 eV)');

% PLOT 4
subplot(2,2,4)
chi2_s = SPix.TableMedSys(:,13); dof_s = SPix.TableMedSys(1,14);
hold on
[~,Nbins,Wbins] = nhist(chi2_s);
binWidth = Wbins(2) - Wbins(1);
x_N = linspace(min(chi2_s),max(chi2_s),...
...
export_fig('plots/pixelSys.pdf','-pdf');
