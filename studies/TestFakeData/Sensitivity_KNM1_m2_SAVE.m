...
% ASIMOV
%M = MultiRunAnalysis('DataType','Fake','FakeRunType','Fake1','RunList','KNM1_30d','exclDataStart',1,'fixPar','5 6','Debug','ON');
...
if exist(savename,'file') && strcmp(RecomputeFlag,'OFF')
    ...
    Mass2      = zeros(7,1); % Single Sys: Stat, TC, TASR, FSD, RF
    Mass2Multi = zeros(7,1); % Multi: Stat, Stat+TC+TASR, +FSD, +RF
...
%     M.FitCM_Obj.WGTS_TASR_RelErr = 1e-3;
    ...
    M.ComputeCM('SysEffect',struct('TCoff_Rad','ON','TCoff_OTHER','ON'),'Stack',StackCM);
    M.Fit;
    Mass2(2) = M.FitResult.err(1);
    Mass2Multi(2) = M.FitResult.err(1);
    
    fprintf(2,'Fit: Stat + TASR + Stack \n');
    M.ComputeCM('SysEffect',struct('TASR','ON'),'WGTS_TASR_RelErr',1e-3,'Stack',StackCM);
    M.Fit;
    Mass2(3) =  M.FitResult.err(1);
    
    fprintf(2,'Fit: Stat + FSD + Stack \n');
    M.ComputeCM('SysEffect',struct('FSD','ON'),'Stack',StackCM);
    M.Fit;
    Mass2(4) = M.FitResult.err(1);
    
    fprintf(2,'Fit: Stat + RF_BF + Stack \n');
    M.ComputeCM('SysEffect',struct('RF_BF','ON'),'Stack',StackCM);
    M.Fit;
    Mass2(5) = M.FitResult.err(1);
    
    fprintf(2,'Fit: Stat + RF_EL + Stack \n');
    M.ComputeCM('SysEffect',struct('RF_EL','ON'),'Stack',StackCM);
    M.Fit;
    Mass2(6) = M.FitResult.err(1);
    
    fprintf(2,'Fit: Stat + RF_RX + Stack \n');
    M.ComputeCM('SysEffect',struct('RF_RX','ON'),'Stack',StackCM);
    M.Fit;
    Mass2(7) = M.FitResult.err(1);
    
    %% With Stacked Systematics
    fprintf(2,'Fit: Stat + TCoff_Rad + TCoff_OTHER + TASR + Stack \n');
    M.ComputeCM('SysEffect',struct('TCoff_Rad','ON','TCoff_OTHER','ON','TASR','ON'),'WGTS_TASR_RelErr',1e-3,'Stack',StackCM);
    M.Fit;
    Mass2Multi(3) = M.FitResult.err(1);
    
    fprintf(2,'Fit: Stat + TCoff_Rad + TCoff_OTHER + TASR + FSD + Stack \n');
    M.ComputeCM('SysEffect',struct('TCoff_Rad','ON','TCoff_OTHER','ON','TASR','ON','FSD','ON'),'WGTS_TASR_RelErr',1e-3,'Stack',StackCM);
    M.Fit;
    Mass2Multi(4) = M.FitResult.err(1);
    
    fprintf(2,'Fit: Stat + TCoff_Rad + TCoff_OTHER + TASR + FSD + RF_BF + Stack \n');
    M.ComputeCM('SysEffect',struct('TCoff_Rad','ON','TCoff_OTHER','ON','TASR','ON','FSD','ON','RF_BF','ON'),'WGTS_TASR_RelErr',1e-3,'Stack',StackCM);
    M.Fit;
    Mass2Multi(5) = M.FitResult.err(1);
    
    fprintf(2,'Fit: Stat + TCoff_Rad + TCoff_OTHER + TASR + FSD + RF_BF + RF_EL + Stack \n');
    M.ComputeCM('SysEffect',struct('TCoff_Rad','ON','TCoff_OTHER','ON','TASR','ON','FSD','ON','RF_BF','ON','RF_EL','ON'),'WGTS_TASR_RelErr',1e-3,'Stack',StackCM);
    M.Fit;
    Mass2Multi(6) = M.FitResult.err(1);
    
    fprintf(2,'Fit: Stat + TCoff_Rad + TCoff_OTHER + TASR + FSD + RF_RX + Stack \n');
    M.ComputeCM('Stack',StackCM,'WGTS_TASR_RelErr',1e-3);
    M.Fit;
    Mass2Multi(7) = M.FitResult.err(1);
       
    %% for stacked plot
    Mass2Stacked = zeros(7,1);
    Mass2Single  = zeros(7,1);
...
leg_str = {' Statistical';'+ TC';'+ Tritium Acitivity Fluctuation';'+ Final State Distribution';...
    '+ Magnetic Fields';'+ Energy Loss Function'; '+ Column Density + Inel. Cross Section'}; %;'+ Response Function'};
...
b = barh([10;20],1.00*[Mass2Stacked , StackEmpty]','stacked');
b(1).LineStyle = 'none';b(2).LineStyle = 'none';b(3).LineStyle = 'none'; b(4).LineStyle = 'none';b(5).LineStyle = 'none'; b(6).LineStyle = 'none';b(7).LineStyle = 'none';
b(1).FaceColor = rgb('White');
...
leg = legend([b(1),b(2),b(3),b(4),b(5),b(6),b(7),pnone,bTC(2),bTASR(2),bFSD(2),bRF_BF(2),bRF_EL(2),bRF_RX(2)],leg_str{:});
leg.Location = 'north';
leg.NumColumns = 2; legend boxoff
PrettyFigureFormat;
ylim([1. 18]);
xlim(1.00*[0.337,0.377]);
grid on;
set(gca,'FontSize',18);
switch Mode
    case 'Sim'
...
mstat = Mass2Multi(1);
msys  = sqrt(Mass2Multi(7)-Mass2Multi(1));
strms = sprintf('m^2 = 0 \\pm %0.2f (stat) \\pm %0.2f (sys) eV^2 \n',mstat,msys);
text(0.35,12.5,strms,'FontSize',20,'FontWeight','bold','FontName','Arial');
