...
%
    ...
    % Parser
    %No Explanation for the Variables :(
    p = inputParser;
    p.addParameter('fign',1,@(x)isfloat(x) && x>0); %figure number
    p.addParameter('pub',@(x)ismember(x,{'ON','OFF','YES'}));
    p.addParameter('display','ON',@(x)ismember(x,{'ON','OFF'}));
    p.addParameter('Mode','Data',@(x)ismember(x,{'Data','Sim','MoSFitterJuly2017'})); %Mode: Fitting Data or Simulation
    ...
    p.addParameter('Fitter','Minuit',@(x)ismember(x,{'Minuit','Matlab'})); %Fitting Software
...
p.parse(varargin{:});
    ...
    switch TD
        case  'KrL3_32_Satellites'
            A=InitKrKATRIN_Lisa('TD',TD,'FPD_Segmentation','OFF',...
        ...
        case {'KrK32','KrL3_32','KrL3_32_HS'}
            A=InitKrKATRIN_Lisa('TD',TD,'FPD_Pixel',Pixel,...
                'ConvFlag',ConvFlag,...
...
%     end
    ...
    for f=1:1:nfit
        ...
        switch Mode
            case 'Data'
                Count    = (krdatamap(Pixel,1,qumin:end)); %consider only 1 pixel
        ...
        switch Fitter
            case 'Minuit'
                ...
                fit_p(:,f)=par;  fit_xi2min(f) = chi2min;  %%%%why?
