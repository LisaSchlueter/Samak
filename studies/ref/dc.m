function [par , err , chi2min, ndof , fig] = dc(varargin)
%
...
A.L3_32_E_i      = 30472.7000;
A.L3_32_W_i      = 1.2199;
A.L3_32_Phi0_i   = 55.4652;
A.BKG_RateSec_i  = 6.9980;
...
% Read Data
f=importdata('../../krypton-data/DataChallenge18V1/kdc_v4_ssc_with_revised_amp_no_fluctuation.txt');
%f=importdata('../../krypton-data/DataChallenge18V1/kdc_v2_fitness_asimov.txt');
A.qU     = f(:,1);
Count    = f(:,2);
CountErr = f(:,3);
Data = [A.qU(:),(Count(:)),(CountErr(:))];
disp(Data);
fprintf(2,'DummyKrdc1: Data - Min=%.2f Max=%.2f Ndata=%.0f\n',...
    min(Count(:)), max(Count(:)),numel(Count(:)));

% Set Time to 1 s 
A.TimeYear = 1/365.2425*43/86400 ; A.TimeSec = 1;
fprintf(2,'DummyKrdc1: Time=%.2f sec\n',A.TimeSec);

% Set qUfrac to 1 per bin
A.qUfrac  = ones(numel(A.qUfrac),1);
fprintf(2,'DummyKrdc1: Fraction of time in qU bins = %s \n',...
    num2str(A.qUfrac'));

% Background / Amplitude Initialization
LinePhi0_i = A.L3_32_Phi0_i;
BKG_i      = A.BKG_RateSec_i ;
fprintf(2,'DummyKrdc1: L3_32_Phi0_i=%.2f \t BKG_RateSec_i=%.2f \n',...
...
%tmparg = sprintf(['set pri -10 ; fix 5 ; set now; minos ; imp ; sho']);
...
%tmparg = sprintf([' fix 5 ; minos  ; SCAn 1 ; SCAn 2 ; SCAn 3 ; SCAn 4 ']);
tmparg = sprintf([' fix  5 ;minos;']);
DataL = {Data,A};
