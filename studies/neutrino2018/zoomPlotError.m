...
function [p z] = zoomPlotError(x,y,e,xbounds,pos,varargin)
...
z = axes('position',pos);
box on 
%h1=errorbar(x,y,e,'o','MarkerSize',3,'MarkerEdgeColor' , [0 0 0]);
h1=errorbar(x,y,e,'ko');
h1.MarkerSize = 3; h1.CapSize = 0;h1.LineStyle= 'none';h1.LineWidth= 2;legend hide
axis([xbounds(1) xbounds(2) 0.5*min(y(index)) max(y(index))]);
end