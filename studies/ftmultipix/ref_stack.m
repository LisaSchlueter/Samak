function myobj = ref_stack(varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% CONFIGURATION FILE
...
p.addParameter('WGTS_CD_MolPerCm2',5e17*0.9);
...
p.addParameter('BKG_RateAllFPDSec',3.364635822262940779e-01,@(x)isfloat(x) && x>=0);
p.addParameter('mnuSq_i',0,@(x)isfloat(x));

% WGTS: Flags for FSD: T-T / D-T / H-T
p.addParameter('DTFSD','DOSS',@(x)ismember(x,{'OFF','DOSS'}));
p.addParameter('WGTS_MolFrac_DT',0.01,@(x)isfloat(x) && x>0);
...
opt_fpd = {...
    ...
    'FPD_Pixel',pixel,'FPD_MeanEff',0.9};
