function myobj = InitKrKATRIN_KrDataChallenge(varargin)
%
...
% Krypton Mode
p.addParameter('L3_32_Phi0_i',66.3,@(x)isfloat(x) && x>0);
...
p.addParameter('TimeYear',(30*43)/(365*24*60*60),@(x)isfloat(x) && x>0);
%p.addParameter('TimeYear',(1)/(365*24*60*60),@(x)isfloat(x) && x>0);
%p.addParameter('TimeYear',1/1000,@(x)isfloat(x) && x>0);
p.addParameter('TD','DummyKrdc1',@(x)ismember(x,{'DummyKrdc1','DR20','DR30','DR40','DR50','Kle15','Kle15Ext','Kle15Dyn','Flat20','Flat30','Flat50','Flat60','Flat100','KrK32','KrL3_32','KrL3_32_HS','MoSFitterJuly2017'}));
p.addParameter('BKG_RateAllFPDSec',7.3,@(x)isfloat(x) && x>0);  %counts per second per qU
p.addParameter('BKG_RateRingSec',7.3,@(x)isfloat(x) && x>0);
p.addParameter('BKG_RatePixelSec',7.3,@(x)isfloat(x) && x>0);
% FPD
...
switch TD
    ...
    case {'DummyKrdc1'}
        opt_mace = {...
            ...
            'MACE_R_eV', 2.7e-04/4.2*Eline,...%Ba/Bmax * Eline
            'HVRipples','OFF'};
end
