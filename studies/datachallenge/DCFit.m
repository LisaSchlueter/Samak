...
A.ComputeTBDDS();
A.ComputeTBDIS('IStype','SIMPFAST');
toc
% progressbar('Neutrino Mass Fits');

% produce and save spectrum
if strcmp(savespectrum,'ON')
    A = init_tbdallpixels('TimeSec',9465984);
    TimePerBin = A.TimeSec*A.qUfrac(1)/148;
    for ii = 1:1
        A.BKG_RateSecallPixels = 0.2/148;
        A.ComputeTBDDSallPixels();
        A.ComputeTBDISallPixels('IStype','SIMPFAST');
        A.AddStatFluctTBDISallPixels();
        TBDIS = real([A.qU,A.TBDISallPixels,A.TBDISE]);
        save(['DC_Data/samakspectrum148p','','.txt'],'TBDIS','-ascii');
