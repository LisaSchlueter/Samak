function chi2 = chi2_hvdata_allpixels(p,XA)
%
...
m  =  model_hvdata_allpixels(p,A);

% Gaussian
%chi2 = sum(( y(1:numel(x)) - m(1:numel(x))).^2 ./ z(1:numel(x)).^2); %stat
%error only
chi2 = (y(1:numel(x)) - m(1:numel(x)))' * ( FitCovErrMatrix \ ( y(1:numel(x)) - m(1:numel(x)))); %with CovMat 
end