function myobj = ref_tfcm(varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% CONFIGURATION FILE
...
% Binning
p.addParameter('nTeBinningFactor',2,@(x)isfloat(x) && x>0);

% KATRIN GENERAL SETTINGS
p.addParameter('TimeSec',24*60*60,@(x)isfloat(x) && x>0);
p.addParameter('WGTS_CD_MolPerCm2',2.5e17);
p.addParameter('Mode','Sim',@(x)ismember(x,{'DataTBD', 'Sim', 'Read'}));
p.addParameter('TDProfile','flat');
p.addParameter('qUmin',17575,@(x)isfloat(x) && x>0);
p.addParameter('qUmax',18580,@(x)isfloat(x) && x>0);
p.addParameter('qUStepSize',5,@(x)isfloat(x) && x>0);
