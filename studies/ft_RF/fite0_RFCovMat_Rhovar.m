...
for ii =1:1:Ntimes 
    A=InitKatrin_ft_RF('WGTS_CD_MolPerCm2', WGTS_CD_MolPerCm2_local(ii),'TD', mtd, 'TimeSec',runtimes);
% Run Parameters
    ...
    C.ComputeCM_RF; %always uses right statistic even if recompute flag 'OFF'
    CovMat_rho{ii,1} = C.MultiCovMat.CM_RF;
    C.ComputeCM_STAT;
   % C.PlotCM('saveplot','ON');
    %C.DecomposeCM('saveplot','ON');
    
    % Define Fit Covariance Matrix : Stat. + Syst.
    FitCovMat = C.MultiCovMat.CM_STAT;%+ C.MultiCovMat.CM_RF;
    
    % Define Model, again...
    A=InitKatrin_ft_RF('WGTS_CD_MolPerCm2', WGTS_CD_MolPerCm2_local(ii),'TD', mtd, 'TimeSec',runtimes,'BKG_Flag','ON'); 
    A.ComputeTBDDS(); A.ComputeTBDIS('IStype','SIMP'); 

    % Loop of fits
    progressbar('E0,m2,B,N Fits');
    DoF = A.nqU - 4;  
    D = InitKatrin_ft_RF('WGTS_CD_MolPerCm2', WGTS_CD_MolPerCm2_local(ii)*(1+0*WGTS_CD_MolPerCm2_RelErr),'TD', mtd, 'TimeSec',runtimes);
    for kk = 1:nfit
    ...
    if nfitsNotConverged(ii) == nfit %when no fit converges
        ...
        title(['(E0)_{eff} - ',mtd,...
            ...
            ' Column Density =',num2str(100*WGTS_CD_MolPerCm2_local(ii)/(5e17)),'%']);
        ...
        fig1str_name = sprintf('E0eff_%s_%gs_%gfits_rho%g_%.2ferr_fit.pdf',mtd,runtimes,nfit,WGTS_CD_MolPerCm2_local(ii),WGTS_CD_MolPerCm2_RelErr);
        ...
        export_fig(fig11str);
        fig2str_name = sprintf('E0eff_%s_%gs_%gfits_rho%g_%.2ferr_chi2.pdf',mtd,runtimes,nfit,WGTS_CD_MolPerCm2_local(ii),WGTS_CD_MolPerCm2_RelErr);
...
figure(999)
subplot(2,1,1)
strtitle = sprintf('(E0)eff - MTD=%s - RunTime=%.1f h - %g fits - CD %g mol/cm^2',mtd,runtimes/(60*60),nfit,max(WGTS_CD_MolPerCm2_local));
errorbar(100*WGTS_CD_MolPerCm2_local/(5e17),muhat*1e3,sigmahat*1e3,'s','MarkerSize',5,'MarkerEdgeColor','black','MarkerFaceColor','blue','LineWidth',2,'Color','Black');
grid on
...
subplot(2,1,2)
stairs(100*WGTS_CD_MolPerCm2_local/(5e17),sigmahat*1e3,'s','MarkerSize',1,'MarkerEdgeColor','black','MarkerFaceColor','black','LineWidth',2);
grid on
xlabel('Nominal Column Density (%)')
...
set(gca,'FontSize',16);
hold on
xx = min(100*WGTS_CD_MolPerCm2_local/(5e17)):0.1:max(100*WGTS_CD_MolPerCm2_local/(5e17));
yy = spline(100*WGTS_CD_MolPerCm2_local/(5e17),sigmahat*1e3,xx);
plot(100*WGTS_CD_MolPerCm2_local/(5e17),sigmahat*1e3,'s',xx,yy,'LineWidth',2,'Color','black')
hold off
PrettyFigureFormat
fig999str_name = sprintf('E0eff_%s_%gs_%gfits_RFfluctuation_columndensity.pdf',mtd,runtimes,nfit);
