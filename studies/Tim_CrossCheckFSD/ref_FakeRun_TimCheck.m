function TritiumObject = ref_FakeRun_TimCheck(varargin)
TDMode = 'DataTBD';

Q = 18575;
qU = linspace(Q-100,Q,10)';
qUfrac = ones(1,10)./10;
% ---------------------------------------------------------------------- %
% WGTS
p = inputParser;
...
p.addParameter('WGTS_MolFrac_TT',1.0,@(x)isfloat(x) && x>0);
...
p.addParameter('Q_i',Q,@(x)isfloat(x));
