function myobj = init_bootcampR35410(varargin)
%
...
p.addParameter('TimeSec',9113,@(x)isfloat(x) && x>0);
p.addParameter('Mode','DataTBD',@(x)ismember(x,{'DataKr','DataTBD','Sim'}));
p.addParameter('TD','35410');%@(x)ismember(x,{'DR20','DR30','DR40','DR50','Kle15','Kle15Ext','Kle15Dyn',...
   % 'Flat5','Flat20','Flat30','Flat50','Flat60','Flat100','BootCampSanshiro'}));
...
p.addParameter('KTFFlag','SSC_BC',@(x)ismember(x,{'OFF','SSCW_DCOfficial','SSC','MACE','MACE+WGTSIS','TL','SG','Kle','MACE+RIPPLE'}));
...
opt_wgts = {...
    'WGTS_CosMaxAAngle',0.3780,...
    'WGTS_Tp',T2purity,...
    'WGTS_DTHTr',1,...
    'WGTS_FTR_cm',4.1,...
    'WGTS_CD_MolPerCm2',1e17/148,...
    'WGTS_B_T',3.6};

opt_mace = {...
    'MACE_Bmax_T',4.2,...
    'MACE_Ba_T',5.8305e-04,...
    'MACE_R_eV',0.93};
