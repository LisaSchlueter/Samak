function [par,err,chi2min,ndof] = fit_tbdallpixelssim_k(varargin)
%
...
p.addParameter('Mode','Bootcamp',@(x)ismember(x,{'Bootcamp','Simulation'}));
...
p.addParameter('nPixels',4,@(x)isfloat(x) && x<150);
    ...
    case 'Bootcamp'
        switch Run
            ...
            case 3541035422
                ...
                D = load('data/runmat35410_35422.mat');
                figprefix = 'bc_fitallsim_run354103_5422';
        end
        A.TimeSec = D.TimeSec; A.qUfrac = (D.qUfrac); A.ComputeNormFactorTBDDS;
    case 'Simulation'
...
switch Backgrounds
    case 'FIT'
        [~ , A.BKG_RateSecallPixels_i] = fit_tbdallpixels(...
            'display','ON','displayPlot','OFF',...
            'nPixels',nPixels,'StatFluct',StatFluct,'mnuSq_t',mnuSq_t);
        %         switch FPDView
        %             case 'ON'
        %                ana_tbdallpixels();
        %        end
        %pause(5);
    case 'READ'
        fitr = load(sprintf('results/bc_fitallpixels_run3541035422.mat'));
...
switch Mode
    ...
    case 'Simulation'
        Data = [...
            ...
            reshape(cell2mat(A.TBDISallPixels),1,A.nqU*A.nPixels)', ...
            reshape(cell2mat(A.TBDISEallPixels),1,A.nqU*A.nPixels)'];
        DataL = {Data,A};
end
switch display
    case 'ON'
...
tmparg = ['set pri -1; fix 1 2; ' FixParStr ' ; min'];
...
[par, err, chi2min, errmat] = pminuit('chi2_tbd_allpixelssim_P',Args);
