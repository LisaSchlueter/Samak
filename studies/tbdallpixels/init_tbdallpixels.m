function myobj = init_tbdallpixels(varargin)
%
...
p.addParameter('TimeSec',94672800,@(x)isfloat(x) && x>0);
p.addParameter('Mode', 'Sim', @(x)ismember(x,{'DataKr', 'DataTBD', 'Sim', 'Read'}));
...
p.addParameter('BKG_RateAllFPDSec',200e-3,@(x)isfloat(x) && x>=0);
...
p.addParameter('KTFFlag','SSC_BC',@(x)ismember(x,{'OFF','SSC','MACE','MACE+WGTSIS','TL','SG','Kle','MACE+RIPPLE'}));
...
opt_wgts = {...
    ...
    'WGTS_DTHTr',1,...
    'WGTS_FTR_cm',4.1,...
    'WGTS_CD_MolPerCm2',5e17/nPixels,...
    'WGTS_B_T',3.6};
