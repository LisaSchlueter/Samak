function myobj = InitKrKATRIN_Fit(varargin)
%
...
% Doppler
p.addParameter('DopplerEffectFlag','numConv',@(x)ismember(x,{'OFF','matConv','Voigt','numConv'}));
p.addParameter('WGTS_Temp',100,@(x)isfloat(x) && x>0);
p.addParameter('DE_sigma',0.058,@(x)isfloat(x) && x>0); % 0.134429846285963
%p.addParameter('DE_sigma',0.5,@(x)isfloat(x) && x>0); % 0.134429846285963
...
opt_katrin = {...
    ...
    'TimeSec',TimeYear*365.25*60*70*24};
...
switch TD
    ...
    case 'MoSFitterJuly2017'
        opt_doppler = {'DopplerEffectFlag',DopplerEffectFlag,...
            'recomputeKernel','OFF','DE_sigma',0.5,'Eplus',4,'Eminus',4};
end
