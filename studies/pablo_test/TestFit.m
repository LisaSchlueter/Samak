...
figure(1);
hold on
plot(A.Te,A.TBDDS,B.Te,B.TBDDS);
legend('numConv','matConv');
hold off

figure(2);
hold on
rd = (A.TBDIS-B.TBDIS)./(0.5*(A.TBDIS+B.TBDIS));
plot(A.qU,rd);
hold off
end
% produce and save spectrum
if strcmp(savespectrum,'ON')
    TimePerBin = A.TimeSec*A.qUfrac(1);
    for ii = 1:1
        A.ComputeTBDDS();
        A.ComputeTBDIS();
        TBDDS = real([A.Te-18575,A.TBDDS]);
        TBDIS = real([A.qU-18575,A.TBDIS/TimePerBin]);
        %save(['samakIntegral10mcps','','.txt'],'TBDIS','-ascii');
        save(['samakDifferentialT2DT','','.txt'],'TBDDS','-ascii');
...
%             DataWithHeaders.data(:,3)];
        ...
        Data = [B.qU,B.TBDIS/(A.qUfrac(1)*A.TimeSec),B.TBDISE/(A.qUfrac(1)*A.TimeSec)];
